...
const Dashboard = () => {
  ...
  const sortedUsers = useMemo(() => {
    const getTimestamp = (user: (typeof users)[number]) =>
    ...
    sorted.sort((a, b) => {
      ...
      if (sortKey === "is_active" || sortKey === "is_client") {
        ...
        comparison = (a[sortKey] ?? "").localeCompare(
          b[sortKey] ?? "",
          undefined,
          { sensitivity: "base" },
        );
      ...
      <div className="mb-5 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">
            Data loaded from the `fetch_users` endpoint.
          </p>
        </div>
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <button
                  type="button"
                  onClick={() => toggleSort("user_phonenumber")}
                  className="text-left font-semibold hover:underline"
                >
          ...
          <TableBody>
            {sortedUsers.map((user, index) => (
              <TableRow key={`${user.user_email}-${index}`}>
                ...
                <TableCell className="text-[var(--muted-foreground)]">
                  {formatTimestamp(getTimestamp(user))}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
