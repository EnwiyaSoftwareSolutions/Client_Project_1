...
import type { Review } from "../../store/useReviews";
      ...
      <div className="mx-auto w-full">
        ...
        {reviews.length > 0 && (
          <div className="review-marquee relative left-1/2 right-1/2 w-screen -translate-x-1/2">
            ...
            >
              {marqueeReviews.map((r: Review, index: number) => (
                <div
                  key={`${r.id}-${index}`}
                  className="review-card relative bg-gradient-to-br from-[var(--boxgradient-color)]/20 via-[var(--primary-accent)]/8 to-transparent border border-[var(--setBorderColorGold)] rounded-xl shadow-md p-6 flex flex-col items-start backdrop-blur-sm"
                >
                  <p className="text-[var(--muted-foreground)] italic mb-4">
                    {r.comment}
                  </p>
                  <p className="text-[var(--headder-text-color)] font-semibold">
                    - {r.reviewer_name}
                  </p>
                  <svg
                    className="absolute right-6 top-6 w-6 h-6 text-[var(--primary-accent)] opacity-10 pointer-events-none select-none"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    aria-hidden
                  >
